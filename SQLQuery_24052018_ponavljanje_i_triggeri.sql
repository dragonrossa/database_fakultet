...
------------------------------------------------------------------------------------------------------------------------------------
 ...
 --1. kreirati funkciju koja raèuna za uneseni matièni broj studenta broj ispita i prosjeènu ocjenu, TESTIRATI NA 1127

 --2. kreirati proceduru koja ispisuje nastavnike za unešenu šifru županije, TEST NA 21

 --3. koliko ispita su položili studenti koji stanuju u mjestu èiji naziv poèinje slovom "Z"

 --4. odredite broj razlièitih nastavnika koji su ispitivali studente tijekom sijeènja ili srpnja bilo koje godine


 --Zadatak 1.

 CREATE FUNCTION FN_STUD_ISPITI (@MBR INT)
 RETURNS TABLE
 AS  
	(SELECT COUNT(*) AS Broj_ispita, AVG(CAST(ocjena AS float) AS PROS
	FROM ispit WHERE mbrStud = @MBR)
RETURN 
END
...
--Zadatak 3.
SELECT COUNT(*) AS BROJ_ISPITA
FROM ispit I JOIN stud S ON s.mbrStud=i.mbrStud
			JOIN mjesto M ON m.pbr=s.pbrStan
			WHERE I.ocjena > 1 AND m.nazMjesto LIKE 'Z%'						--rezultat je 155


--Zadatak 4.
SELECT COUNT(DISTINCT sifNastavnik) AS BROJ_NAST
FROM ispit I
WHERE MONTH(DATISPIT) IN (1,7)      --rezultat je 29
...
AS
PRINT 'Ne mijenjati podatke'

update orgjed
set sifOrgjed = 999
where sifOrgjed = 100001

select * from orgjed

--Zadatak 5.12

CREATE TRIGGER OBAVIJEST
ON stud
AFTER INSERT
AS
PRINT 'Dodan je novi student'

--test
SELECT * FROM stud
INSERT INTO stud VALUES (999,'Pero','Periæ',10000,10000, '2000-01-01', '123456789')

--DODAN JE NOVI STUDENT >>>>>> - TAB messages - (305 row(s) affected) Dodan je novi student (1 row(s) affected)

--Zadatak 5.14

CREATE TRIGGER ProvjeraDatuma
ON ispit
FOR INSERT															  -- INSERT/UPDATE/DELETE
AS
IF (SELECT datIspit FROM inserted) BETWEEN GETDATE() AND GETDATE()+30 -- inserted/updated/deleted
	BEGIN
		ROLLBACK
	END
...
INSERT INTO ispit VALUES (999,229,122,'2018-06-01',5) -- Msg 3609, Level 16, State 1, Line 1056 The transaction ended in the trigger. The batch has been aborted. 
-- ako je datum izmeðu današnjeg i današnjeg+30 dana, sve ostalo unosi
...
CREATE TRIGGER NOVISTUDENT
ON STUD
INSTEAD OF INSERT
AS
BEGIN
	INSERT INTO NOVISTUDENT         --s obzirom da tablica NOVISTUDENT ne postoji ovaj trigger æe ju kreirati
		SELECT * FROM inserted
END

--ispisati sve studente koji nisu polagali ispite na najmanje 3 razlièita naèina:
...
SELECT s.imeStud, s.prezStud
FROM stud S LEFT OUTER JOIN ispit I ON
s.mbrStud=i.mbrStud
WHERE i.ocjena IS NULL             --rezultat je 195
...
CREATE INDEX MBR_IDX ON ISPIT(MBRSTUD)

-------

SELECT s.imeStud, s.prezStud
FROM stud s WHERE s.mbrStud NOT IN (SELECT DISTINCT mbrStud FROM ispit)   --rezultat je 195, kad bi stavili samo IN rez bi bio broj studenata

SELECT *
FROM stud S
WHERE NOT EXISTS (SELECT 1 from ispit I WHERE s.mbrStud=i.mbrStud)



 ISPIT 
 Za rješavanje ispita koristite bazu podataka Fakultet.  
 
ZADATAK 1 
 
Svaki nastavnik ima u tablici nastavnik upisan poštanski broj mjesta u kojem živi. 
Ispišite koliko nastavnika živi u svakom pojedinom mjestu. Ujedno i ispišite nazive tih mjesta. 
 
ZADATAK 2 
 
Ispišite podatke o studentima koji su polagali ispit kod nekog od profesora koji radi u Zavodu za primjenjenu matematiku. 
Upit rješite koristeæi podupit. 
 
ZADATAK 3 
 
Kreirajte pogled koji ispisuje sve šifre i nazive organizacijskih jedinica kojima pripadaju ispiti koje su studenti polagali. 
 
ZADATAK 4 
 
Napišite pohranjenu proceduru koja za traženog studenta (preko matiènog broja) daje rezultat koliko je student ispita položio. 
Procedura mora vratiti vrijednost 0 za studenta koji nije položio niti jedan ispit ili nikad nije niti izašao na ijedan ispit. 
 
ZADATAK 5 
 
Napišite okidaè koji ne dozvoljava brisanje tablice Pred i ispisuje poruku 'Tablicu predmeta nije moguæe obrisati' nakon pokušaja brisanja

--1. Svaki nastavnik ima u tablici nastavnik upisan poštanski broj mjesta u kojem živi. 
--	 Ispišite koliko nastavnika živi u svakom pojedinom mjestu. Ujedno i ispišite nazive tih mjesta. 

SELECT COUNT(pbrStan) as Mjesto_stanovanja
FROM nastavnik N JOIN mjesto M ON n.pbrStan=m.nazMjesto
where pbrStan IN (SELECT N.imeNastavnik,N.prezNastavnik, floor(n.pbrStan), CEILING(n.pbrStan)
FROM Nastavnik N)

--2. Ispišite podatke o studentima koji su polagali ispit kod nekog od profesora koji radi u Zavodu za primjenjenu matematiku. 
--   Upit rješite koristeæi podupit. 

SELECT s.* 
FROM stud S JOIN ispit I ON s.mbrStud=i.mbrStud
			WHERE EXISTS (SELECT n.* FROM ispit I JOIN nastavnik N  ON I.sifNastavnik=n.sifNastavnik WHERE o. LIKE 'zavod za primijenjenu matema%')

--3. Kreirajte pogled koji ispisuje sve šifre i nazive organizacijskih jedinica kojima pripadaju ispiti koje su studenti polagali. 

CREATE VIEW STUDENTI_KOJI_SU_IZAŠLI_NA_ISPIT_IZ_ORGANIZACIJSKIH_JEDINICA
AS
SELECT o.sifOrgjed, o.nazOrgjed
FROM orgjed o JOIN pred p
ON o.sifOrgjed=p.sifOrgjed
WHERE o.sifOrgjed IN (SELECT o.sifOrgjed FROM ispit I WHERE i.ocjena IS NOT NULL)
